...
def fetch_data(ticker: str, bartime: str, start: int):
    bartimes_convert = {'1_day': 86400, '4_hour': 14400, '1_hour': 3600, '15_minute': 900, '5_minute': 300, '1_minute': 60}
    ...
    response = response.get(url, params={'start': start, 'end': end, 'granularity': bartime})
    ...
    if response.status_code != 200:
        ...
        if candles is not None:
            ...
            candles_df['date'] = candles_df['date'].apply(lambda x: pd.to_datetime(x))
